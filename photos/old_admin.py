...
class BatchAdmin(admin.ModelAdmin):
    ...
    photo_count_display.short_description = 'Photo Status'
    ...
    def zip_status_display(self, obj):
        ...
        color = status_colors.get(obj.zip_status, 'gray')
        
        html = format_html(
            '<span style="color: {}; font-weight: bold;">{}</span>',
            color,
            obj.get_zip_status_display()
        )
        
        if obj.zip_status == 'failed' and obj.zip_error:
            html += format_html(
                '<br/><small style="color: red;" title="{}">{}</small>',
                obj.zip_error,
                obj.zip_error[:50] + '...' if len(obj.zip_error) > 50 else obj.zip_error
            )
        
        return html
    zip_status_display.short_description = 'ZIP Status'
    ...
    def zip_file_link(self, obj):
        ...
        if obj and obj.zip_file:
            try:
                ...
                return format_html(
                    '<a href="{}" target="_blank" class="button">Download ZIP</a>',
                    url
                )
            except:
                return 'ZIP file exists but URL unavailable'
        
        if obj.zip_status == 'processing':
            return format_html('<em>ZIP file is being generated...</em>')
        elif obj.zip_status == 'failed':
            return format_html('<em style="color: red;">ZIP generation failed</em>')
        
        return 'No ZIP file yet'
    zip_file_link.short_description = 'ZIP File'
    ...
    regenerate_zip_files.short_description = "Regenerate ZIP files"
    ...
    regenerate_all_previews.short_description = "Regenerate all previews"
    ...
    def view_photos_link(self, obj):
        ...
        return format_html(
            '<a href="{}" class="button">View {} Photo(s)</a>',
            url,
            count
        )
    view_photos_link.short_description = 'Photos'
...
class PhotoAdmin(admin.ModelAdmin):
    ...
    def batch_link(self, obj):
        ...
        return format_html('<a href="{}">{}</a>', url, obj.batch.title)
    batch_link.short_description = 'Batch'
    
    def preview_thumbnail(self, obj):
        """Display small thumbnail"""
        if obj.id:
            url = obj.get_thumbnail_url(width=80, height=80)
            if url:
                return format_html(
    ...
    preview_thumbnail_large.short_description = 'Preview Image'
    ...
    def preview_error_display(self, obj):
        ...
        return "No errors"
    preview_error_display.short_description = 'Error Details'
    ...
    retry_preview_generation.short_description = "Retry preview generation"
    ...
    force_delete_previews.short_description = "Delete and regenerate previews"
...
#         if obj.id:
#             url = obj.get_thumbnail_url(width=100, height=100)
#             if url:
#                 return format_html(
